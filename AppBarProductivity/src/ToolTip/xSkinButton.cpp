...
void CxSkinButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	CRect r=lpDrawItemStruct->rcItem;					//+++hd // context rectangle
	//int cx = r.right  - r.left ;						// get width
	//int cy = r.bottom - r.top  ;						// get height
	// +++hd  get text box position
	CSize sz = pDC->GetTextExtent(sCaption);
	RECT tr={r.left+m_FocusRectMargin+2,r.top+(r.Height()-2*sz.cy)/2,
		r.right-m_FocusRectMargin-2,r.bottom};
				  ...
				  short drawmode, short border, short margin)
{
	...
	m_DrawMode=(short)max(0,min(drawmode,2));
	m_Border=border;
	m_FocusRectMargin=(short)max(0,margin);
...
//
//---------------------------------------------------------
LRESULT CxSkinButton::OnRadioInfo(WPARAM, LPARAM)
{
...
/////////////////////////////////////////////////////////////////////////////
LRESULT CxSkinButton::OnBMGetCheck(WPARAM, LPARAM)
