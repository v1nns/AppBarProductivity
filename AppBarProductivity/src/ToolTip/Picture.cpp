...
bool CPicture::Load(LPCTSTR sResourceType, UINT nResource)
{
	bool bResult = false;
	...
	if (m_pPicture == NULL)
	{
		...
		if (GetResource(nResource, sResourceType, pBuff, nSize))
		{
			if (nSize > 0)
			{
...
bool CPicture::Load(LPCTSTR sResourceType, LPCTSTR sResource)
{
	bool bResult = false;
...
bool CPicture::Load(LPCTSTR sFileName)
{
	bool bResult = false;
...
bool CPicture::GetResource(UINT nResource, LPCTSTR lpType, BYTE* pResource, int& nBufSize)
{ 
	HRSRC		hResInfo;
	HANDLE		hRes;
	HMODULE		hInst	= NULL; 
	...
	// Find the resource
	hResInfo = FindResource(hInst, MAKEINTRESOURCE(nResource), lpType);
...
bool CPicture::GetResource(LPCTSTR lpName, LPCTSTR lpType, BYTE* pResource, int& nBufSize)
{ 
	HRSRC		hResInfo;
	HANDLE		hRes;
	HMODULE		hInst	= NULL; 
