//  Updated:    31 July 2006  by Hans Dietrich for XSuperTooltip
...
void CPPHtmlDrawer::SetListOfTags()
{
	...
	AddTagToList(_T("indent"), TAG_INDENT, _T("indent"));	// +++hd
...
LPLOGFONT CPPHtmlDrawer::GetSystemToolTipFont() const
{
	...
	CPPTOOLTIP_TRACE(_T("lf.lfFaceName=%s =========\n"), lf.lfFaceName);
...
void CPPHtmlDrawer::JumpToHyperlink(int nLink, int nIDTool)	//+++hd
{
	STRUCT_HYPERLINK & link = m_arrLinks [nLink];
	CPPTOOLTIP_TRACE(_T("Jump to Hyperlink number = %d\n"), nLink);
...
void CPPHtmlDrawer::OnLButtonDown(LPPOINT lpClient, int nIDTool)	//+++hd
{
	CPPTOOLTIP_TRACE(_T("CPPHtmlDrawer::OnLButtonDown()\n"));
	
	int nLink = PtInHyperlink(lpClient);
	if (nLink >= 0)
	{
		//Hyperlink under the mouse pointer
		JumpToHyperlink(nLink, nIDTool);		//+++hd
	} //if
} //End OnLButtonDown

BOOL CPPHtmlDrawer::OnSetCursor(LPPOINT lpClient)
{
	CPPTOOLTIP_TRACE(_T("in CPPHtmlDrawer::OnSetCursor\n"));
...
{
	CPPTOOLTIP_TRACE(_T("CPPHtmlDrawer::CallbackOnRepaint()\n")); 
...
{
	CPPTOOLTIP_TRACE(_T("CPPHtmlDrawer::CallbackOnClickHyperlink()\n")); 

	if ((NULL == m_csCallbackLink.hWnd) || !m_csCallbackLink.nMessage)
		return; 
	
	::SendMessage(m_csCallbackLink.hWnd, m_csCallbackLink.nMessage, 
		(LPARAM)sLink, m_nIDTool); //+++hd m_csCallbackLink.lParam);  	
...
//		m_arrTableSizes.clear();
	...
	_tcsncpy(m_lfDefault.lfFaceName, m_defStyle.sFaceFont, 
		sizeof(m_lfDefault.lfFaceName)/sizeof(TCHAR)-1);
	CPPTOOLTIP_TRACE(_T("m_defStyle.sFaceFont=%s =========\n"), m_defStyle.sFaceFont);
	...
	if (m_bUseCustomFont)
	{
		m_hFont = ::CreateFontIndirect(&m_lfCustom);
	}
	else
	{
		m_hFont = ::CreateFontIndirect(&m_lfDefault);
		CPPTOOLTIP_TRACE(_T("m_lfDefault.lfFaceName=%s =========\n"), m_lfDefault.lfFaceName);
	}
	// new jms code ends
				...
				for (nIndex = 0; nIndex < sText.GetLength(); nIndex++)
				{
					if ((sText.GetAt(nIndex) >= _T('a')) && (sText.GetAt(nIndex) <= _T('z')))
					{
						if ((0 == nIndex) || (_T(' ') == sText.GetAt(nIndex - 1)))
							sText.SetAt(nIndex, (TCHAR)(sText.GetAt(nIndex) - _T('a') + _T('A')));
					} //if
				} //if
				break;
			} //switch
								...
								while (nIndex < sProperties.GetLength())
								{
																		...
																		if (!sParameter.IsEmpty())
									{
										//ENG: Processing a parameters of a tag
																				if (sParameter == _T("size"))
											nIndent = GetLengthUnit(sValue, nIndent, TRUE);
									}
								}
								m_defStyle.nIndent = nIndent;
								break;
							case TAG_BOLD:
					...
					else 
					{
						...
						case TAG_HLINE:
							...
							else if (MODE_DRAW == m_nNumPass)
							{
								...
								//+++hd  draw 3D line
								m_drawmanager.DrawLine(m_hDC, ptOutput.x, ptOutput.y + m_hline.nHeightLine / 2 + 1, 
									ptOutput.x + nWidth, ptOutput.y + m_hline.nHeightLine / 2 + 1, 
									RGB(255,255,255), CPPDrawManager::PEN_SOLID, csTemp.nBorderWidth);
								ptOutput.y += 1;
							} //if
							ptOutput.x += nWidth;
							break;
...
int CPPHtmlDrawer::InitNewLine(int x)
{
	...
	return x + m_defStyle.nIndent;		// +++hd
} //End of InitNewLine
...
//+++hd - from XHyperLink.cpp
// old version loaded winhlp32.dll every time
void CPPHtmlDrawer::SetDefaultCursor()
{
	if (m_hLinkCursor == NULL)				// No cursor handle - try to load one
	{
		...
		CPPTOOLTIP_TRACE(_T("loading from IDC_HAND\n"));
											...
											// load the WinHelp hand cursor
		{
			...
			CPPTOOLTIP_TRACE(_T("loading from winhlp32\n"));
...
void CPPHtmlDrawer::SetDefaultCursor()
{
    ...
    {
#ifdef IDC_HAND
		// This code was added from Zorglab's comments to hyperlink control 
		// from Chris Maunder
...
{
	CPPTOOLTIP_TRACE(_T("CPPHtmlDrawer::SetCallbackHyperlink()\n"));
...
{
	CPPTOOLTIP_TRACE(_T("CPPHtmlDrawer::SetCallbackRepaint()\n"));
...
void CPPHtmlDrawer::SetDefaultStyles(_STRUCT_CHANGESTYLE & /*cs*/)
{
	...
	LOGFONT * plf = GetSystemToolTipFont();
	ASSERT(plf);

	if (plf)
	{
		m_defStyle.nSizeFont = plf->lfHeight;		//The height of the logic font
		m_defStyle.nWeightFont = plf->lfWeight;	//The weight of the logic font
		m_defStyle.bItalicFont = plf->lfItalic;	//Is italic logic font?
		m_defStyle.bUnderlineFont = plf->lfUnderline;//Is underline logic font?
		m_defStyle.bStrikeOutFont = plf->lfStrikeOut;//Is strikeout logic font?
		m_defStyle.bOverlineFont = FALSE; //Is overline logic font?
		m_defStyle.sFaceFont = plf->lfFaceName;  //The face name of the logic font
		if (m_defStyle.sFaceFont == _T("Segoe UI"))
		{
			if (m_defStyle.nSizeFont < 0)
				m_defStyle.nSizeFont -= 1;
			else
				m_defStyle.nSizeFont += 1;
		}
	}

	CPPTOOLTIP_TRACE(_T("SetDefaultStyles:  m_defStyle.sFaceFont=%s =========\n"), m_defStyle.sFaceFont);
