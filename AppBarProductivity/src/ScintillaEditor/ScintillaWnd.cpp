...
CScintillaWnd::CScintillaWnd()
{
   ...
   /* Load Accelerator */
   m_hAccel = LoadAccelerators(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDR_RICH_EDIT_ACCELS));
}

/////////////////////////////////////
// @mfunc This is a copy constructor
// @rvalue void | not used
//
CScintillaWnd::CScintillaWnd(const CScintillaWnd& other)
{
	m_bLinenumbers = FALSE;
	m_bSelection = TRUE;
	m_bFolding = FALSE;
...
{
	if (!CWnd::CreateEx(WS_EX_TOOLWINDOW, STR_SCINTILLAWND,lpszWindowName, dwStyle, rect, pParentWnd,(UINT)nID))
   {   
      LPVOID lpMsgBuf;
...
BEGIN_MESSAGE_MAP(CScintillaWnd, CWnd)
	ON_WM_MBUTTONDOWN()
	ON_COMMAND(IDR_RICH_EDIT_TEXT_BOLD, OnTextBold)
END_MESSAGE_MAP()


void CScintillaWnd::OnMButtonDown(UINT nFlags, CPoint point)
{
...
void CScintillaWnd::OnTextBold()
{
	/*Sci_TextRange tr;

	tr.chrg.cpMin = GetSelectionStart();
	tr.chrg.cpMax = GetSelectionEnd();

	CString text = GetSelectedText(); // if necessary...
	tr.lpstrText = (char*) malloc(2 * (tr.chrg.cpMax - tr.chrg.cpMin) + 2);
	
	int i = SendMessage(SCI_GETSTYLEDTEXT, 0, (LPARAM)&tr);
	...
	// go to that position
	SendMessage(SCI_STARTSTYLING, tr.chrg.cpMin, 0);

	if (i == 4 || i == 8)
	{
		// change its style
		SendMessage(SCI_SETSTYLING, tr.chrg.cpMax - tr.chrg.cpMin, 200);
	}
	else if (i == 200)
	{
		SendMessage(SCI_SETSTYLING, tr.chrg.cpMax - tr.chrg.cpMin, 4);
	}*/
}