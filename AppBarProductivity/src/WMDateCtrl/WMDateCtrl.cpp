...
CWMDateCtrl::CWMDateCtrl()
{
	...
	nFontSize = 85;
	strFontName = "TAHOMA";
	bMilTime = TRUE;	// NonMilitary Time
...
void CWMDateCtrl::OnPaint() 
{
	...
	cdc.BitBlt(0, 0, rect.right, rect.bottom, &dc, 0, 0, SRCCOPY);
	dc.SelectObject(*oldBmp);
}

// Draw Headers within each Date Cell
void CWMDateCtrl::DrawWeekCellHeaders(CDC *pDC)
{
...
void CWMDateCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	while (pos != NULL)
	{
		...
		pCellData->SetHighlightEntireArea(FALSE);
	...
	while (pos != NULL)
	{
		...
		if (rect.PtInRect(point) == TRUE)
		{
			...
			else
			{
				...
				{
					// DRAW ENTIRE SELECTED
					pCellData->SetHighlightEntireArea(TRUE);
				}
			}
			break;
		}
...
void CWMDateCtrl::PaintWeekView(CDC *pDC)
{
	CRect rect, rCell;
	COleDateTime dtDate = dtCurrentDate;
...
void CWMDateCtrl::PaintMonthView(CDC *pDC)
{
	...
	int nCol = 0, nRow = 0;
	int nInterval = (int)(rArea.right /6);	// Width of each cell
	...
	for (nRow = 1; nRow <= 6; nRow++)
	{
		...
		for (nCol = 1; nCol <= 6; nCol++)
		{
			...
			else
			{
				...
				rSun.right = rSat.right + 1;
				pDC->FillSolidRect(&rSun, pData->GetBkColor());
...
void CWMDateCtrl::DrawMonthHeader(CDC *pDC)
{
	// Area to paint
	CRect rArea(0,0,0,0);
	GetClientRect(&rArea);
	...
	for (int x = 1; x <= 7; x++)
	{
		...
		pDC->MoveTo(rHdr.left + 1, rHdr.top + 1);
		pDC->LineTo(rHdr.left + 1, rHdr.bottom - 1);
...
void CWMDateCtrl::DrawMonthCellHeaders(CDC *pDC)
{
	CRect rect, rHdr, rArea;
	// Calculate height of Cell Header to be the width of the scroll bars
	int nHdrHeight = GetSystemMetrics(SM_CXVSCROLL);
	...
	for (nItem = 1; nItem <= WV_TOTALCELLS; nItem++)
	{
		...
		if (pCell->GetCellDate() == dtCurrentDate)
		{
			...
			if (pCell->GetHighlight())
			{
				...
				rText.right = rHdr.right;
				rText.bottom = rHdr.bottom;
				pDC->FillSolidRect(&rHdr, clrHL);

				drawTextInWhite = true;
			}
		}	// if (pCell->GetCellDate() == dtCurrentDate)
		else
		{
			// Check if we need to highlight cell as well
			if (pCell->GetHighlightEntireArea())
			{
				CRect cellRect;
				pCell->GetCellRect(&cellRect);

				pDC->FillSolidRect(&cellRect, clrHL);
				
				drawTextInWhite = true;
			}
			else if (pCell->GetHighlight())
			{
				// Normal highlight of header
				rHdr.left -= 1;
				rHdr.top += 1;
