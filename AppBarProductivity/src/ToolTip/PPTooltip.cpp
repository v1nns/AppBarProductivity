...
#include "stdafx.h"
#include "PPTooltip.h"
#include "htmlhelp.h"		//+++hd
#include <io.h>				//+++hd
#include "Monitors.h"		//+++hd
#include "MultiMonitor.h"	//+++hd
...
// note - comment out these lines for static build - +++hd

//#define COMPILE_MULTIMON_STUBS	//+++hd
//#include "multimon.h"				//+++hd
...
//+++hd
UINT WM_PPTOOLTIP_VIRTUAL_KEY_CODE = 
	::RegisterWindowMessage((LPTSTR) _T("WM_PPTOOLTIP_VIRTUAL_KEY_CODE"));
...
{
	// Default values
	m_bIgnoreEnabledStatus = TRUE;	// jms - 07/14/2006	// +++hd
	m_bMouseCaptured = FALSE;		//+++hd
...
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::Create\n"));
...
{ 
	CPPTOOLTIP_TRACE(_T("CPPToolTip::SendNotify()\n")); 
...
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::IsNotify\n"));
...
BOOL CPPToolTip::OnSetCursor(CWnd* /*pWnd*/, UINT /*nHitTest*/, UINT /*message*/) 
{
	...
	CPPTOOLTIP_TRACE (_T("CPPToolTip::OnSetCursor(x=%d, y=%d)\n"), ptClient.x, ptClient.y);
	if (m_drawer.OnSetCursor(&ptClient))
	{
		CPPTOOLTIP_TRACE(_T("cursor over hyperlink\n"));
...
{
	CPPTOOLTIP_TRACE (_T("CPPToolTip::OnRepaintWindow()\n"));
...
{
	CPPTOOLTIP_TRACE (_T("CPPToolTip::OnRedrawTooltip(Transparency = %d)\n"), nTransparency);
...
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::OnPaint()\n"));
...
BOOL CPPToolTip::RelayEvent(MSG* pMsg)
{
	...
	if (m_nTooltipState == PPTOOLTIP_STATE_SHOWN)
	{
		...
		{
			CPPTOOLTIP_TRACE(_T("CPPToolTip::RelayEvent:   WM_KEYDOWN or WM_KEYUP\n"));
			if (pMsg->wParam == m_tiNextTool.nVirtualKeyCode)
			{
				CPPTOOLTIP_TRACE(_T("CPPToolTip::RelayEvent:  virtual key code seen\n"));
				if (pMsg->message == WM_KEYDOWN)
				{
					if (::IsWindow(m_hParentWnd))
					{
						CPPTOOLTIP_TRACE(_T("sending message to parent\n"));
						::PostMessage(m_hParentWnd, WM_PPTOOLTIP_VIRTUAL_KEY_CODE, 
							m_tiNextTool.nKeyCodeId, 
							m_tiNextTool.nIDTool);
					}
					else
					{
						CPPTOOLTIP_TRACE(_T("ERROR: parent window doesn't exist\n"));
					}
				}
		...
		if ((pMsg->message == 0x4d) && (m_tiNextTool.nVirtualKeyCode == VK_F1))
		{
			// discard MFC "magic" help message if the virtual key code is F1
			CPPTOOLTIP_TRACE(_T("CPPToolTip::RelayEvent:  MFC magic help message\n"));
			return TRUE;
		}
	}
	...
	case WM_RBUTTONDOWN:
		...
		break;
	case WM_LBUTTONDOWN:
		CPPTOOLTIP_TRACE(_T("CPPToolTip::WM_LBUTTONDOWN\n"));
		if (IsCursorOverTooltip())
		{
			//Left Button was pressed over the tooltip
			pt = pMsg->pt;
			ScreenToClient(&pt);
			m_drawer.OnLButtonDown(&pt, m_tiNextTool.nIDTool); //+++hd
		} //if
...
//		if (!(m_tiDisplayed.nBehaviour & PPTOOLTIP_NOCLOSE_MOUSEDOWN))
	...
	case WM_MOUSEMOVE:
		...
		if ((m_ptOriginal.x != pMsg->pt.x) || (m_ptOriginal.y != pMsg->pt.y))
		{
			...
			{
				CPPTOOLTIP_TRACE(_T("over tooltip ==========\n"));
			...
			{
				CPPTOOLTIP_TRACE(_T("not over tooltip ==========\n"));
				...
				if (NULL == hWnd)
				{
					...
					if ((m_tiDisplayed.rectBounds.left <= ptClient.x) && (m_tiDisplayed.rectBounds.right >= ptClient.x)
						&& (m_tiDisplayed.rectBounds.top <= ptClient.y) && (m_tiDisplayed.rectBounds.bottom >= ptClient.y))
					{
						//do nothing
					}
					else
				...
				{
					CPPTOOLTIP_TRACE(_T("found tool =========\n"));
					...
					else
					{
						...
						{
							CPPTOOLTIP_TRACE(_T("calling SetAutoPopTimer ==========\n"));
...
{
	CPPTOOLTIP_TRACE (_T("CPPToolTip::SetNewTooltip(hWnd=0x%08X, CRect(left=%d, top=%d, right=%d, bottom=%d), nID=%d)\n"), 
	...
	{
		CPPTOOLTIP_TRACE(_T("SetNewTooltip2(%d)\n"), m_nNextTooltipType);
...
void CPPToolTip::OnTimer(UINT nIDEvent) 
{
	...
	{
	case TIMER_SHOW:
		CPPTOOLTIP_TRACE(_T("OnTimerShow(%d)\n"), m_nNextTooltipType);
		//ENG: Kill SHOW timer 
				KillTimer(TIMER_SHOW);
		...
		if ((pt.x != m_ptOriginal.x) || (pt.y != m_ptOriginal.y))
		{
			...
			CPPTOOLTIP_TRACE(_T("OnTimerShow(HideTooltip)\n"));
			HideTooltip();
		}
		else if (PPTOOLTIP_STATE_HIDEN == m_nTooltipState)
		{
			CPPTOOLTIP_TRACE(_T("OnTimerShow(Showing)\n"));
			...
			{
				CPPTOOLTIP_TRACE(_T("OnTimerShow(FadeIn)\n"));
				SetTimer(TIMER_SHOWING, m_dwTimeFadeIn, NULL); //Fade in showing was enabled
			}
			else
			{
				CPPTOOLTIP_TRACE(_T("OnTimerShow(Shown)\n"));
		...
		break;
	case TIMER_SHOWING:
		CPPTOOLTIP_TRACE(_T("OnTimerFadeIn(Current Transparency=%d )\n"), m_dwCurTransparency);
		...
		break;	
	case TIMER_HIDE:
		CPPTOOLTIP_TRACE(_T("OnTimerHide()\n"));

		if (m_bMouseCaptured)		//+++hd
			ReleaseCapture();		//+++hd
		m_bMouseCaptured = FALSE;	//+++hd
		...
		break;
	case TIMER_HIDING:
		CPPTOOLTIP_TRACE(_T("OnTimerFadeOut(Current Transparency=%d)\n"), m_dwCurTransparency);
...
{
	CPPTOOLTIP_TRACE (_T("CPPToolTip::HideTooltip(CurState=%d)\n"), m_nTooltipState);
...
{
	CPPTOOLTIP_TRACE (_T("CPPToolTip::SetAutoPopTimer()\n"));
...
{
	CPPTOOLTIP_TRACE (_T("CPPToolTip::KillTimers()\n"));
	if (dwIdTimer == NULL)
	{
		KillTimer(TIMER_SHOW);
		KillTimer(TIMER_HIDE);
...
void CPPToolTip::Pop()
{
	CPPTOOLTIP_TRACE (_T("CPPToolTip::Pop()\n"));
...
{
	CPPTOOLTIP_TRACE (_T("CPPToolTip::PrepareDisplayTooltip()\n"));
	...
	// +++hd
	if (m_tiNextTool.bSuperTooltip)
	{
		CPoint ptclient(pt);
		::ScreenToClient(m_hParentWnd, &ptclient);
		HWND hWnd = ::ChildWindowFromPoint(m_hParentWnd, ptclient);
		if (hWnd == NULL)
		{
			pt.x += 24;
			pt.y += 22;
	...
	int nMarginY = m_nSizes[PPTTSZ_MARGIN_CY];
	m_rcTipArea.OffsetRect(m_nSizes[PPTTSZ_MARGIN_CX], nMarginY);
	m_rcTooltip.InflateRect(0, 0, 2 * m_nSizes[PPTTSZ_MARGIN_CX], 2 * m_nSizes[PPTTSZ_MARGIN_CY]);
	if (m_tiNextTool.bSuperTooltip)
	{
		m_rcTipArea.top += nMarginY;
		m_rcTooltip.bottom += nMarginY;
	}

	//Inflates on 
	//Gets tooltip's rect with anchor
	CPoint ptAnchor;
	m_dwCurDirection = GetTooltipDirection(m_tiNextTool.nDirection, pt, 
		ptAnchor, m_rcTooltip, m_rcBoundsTooltip, m_rcTipArea);
	if (m_tiNextTool.bSuperTooltip)
		ptAnchor.x = ptAnchor.y = 0;	//+++hd
	...
	{
		CPoint upper_left(rect.left, rect.top);
		CMonitor mon = CMonitors::GetNearestMonitor(upper_left);
		CRect rMonitor;
		mon.GetMonitorRect(&rMonitor);
		CPPTOOLTIP_TRACE(_T("rMonitor:  left=%d  top=%d  right=%d  bottom=%d --------\n"),
			rMonitor.left, rMonitor.top, rMonitor.right, rMonitor.bottom);
		CPPTOOLTIP_TRACE(_T("rect:  left=%d  top=%d  right=%d  bottom=%d --------\n"),
			rect.left, rect.top, rect.right, rect.bottom);

		if (rect.bottom > rMonitor.bottom)
		{
			CPPTOOLTIP_TRACE(_T("off the screen -----------\n"));
			CRect rTool;
			::GetWindowRect(hWndTool, &rTool);
			rect.top = rTool.top - m_rcBoundsTooltip.Height() + m_szOffsetShadow.cy + 1;
			rect.bottom = rTool.top + m_szOffsetShadow.cy + 1;
			CPPTOOLTIP_TRACE(_T("rect:  left=%d  top=%d  right=%d  bottom=%d ----------\n"),
				rect.left, rect.top, rect.right, rect.bottom);
		}
...
{
	CPPTOOLTIP_TRACE(_T("OutputTooltipOnScreen()\n"));
	CRect rect = m_rcBoundsTooltip;
...
{
	CPPTOOLTIP_TRACE(_T("in CPPToolTip::GetWndFromPoint:  m_hParentWnd=0x%X\n"), m_hParentWnd);
	...
	HWND hWnd = NULL;
	if (m_bIgnoreEnabledStatus)		//+++hd
	{
		// point must be in client coordinates
		hWnd = ::ChildWindowFromPoint(m_hParentWnd, *lpPoint);
		CPPTOOLTIP_TRACE(_T("ChildWindowFromPoint returned 0x%X\n"), hWnd);
	}
	else
	{
		hWnd = ::WindowFromPoint(pt);
		CPPTOOLTIP_TRACE(_T("WindowFromPoint returned 0x%X\n"), hWnd);
	}
	if (NULL != hWnd)
	{
		// try to hit combobox instead of edit control for CBS_DROPDOWN styles
		HWND hWndTemp = ::GetParent(hWnd);
		CPPTOOLTIP_TRACE(_T("GetParent returned 0x%X\n"), hWndTemp);
		if (NULL != hWndTemp)
		{
			if (!IsComboBoxControl(hWndTemp, CBS_DROPDOWN))
			{
				...
				if (NULL == hWndTemp)
				{
					CPPTOOLTIP_TRACE(_T("ChildWindowFromPoint returned NULL\n"));
					return NULL;
				}
				if (!m_bIgnoreEnabledStatus) // <--------- jms - 07/14/2006		// +++hd
				{
			...
			CPPTOOLTIP_TRACE(_T("calling FindTool\n"));
...
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::AddTool(hWnd=0x%08X)\n"), pWnd->GetSafeHwnd());
	ASSERT (pWnd);
...
{
	CPPTOOLTIP_TRACE (_T("CPPToolTip::RemoveTool(hWnd=0x%08X)\n"), pWnd->GetSafeHwnd());
	ASSERT(pWnd);
...
{
	CPPTOOLTIP_TRACE (_T("CPPToolTip::RemoveAllTools()\n"));
...
{
	CPPTOOLTIP_TRACE (_T("CPPToolTip::AddToolBar(hWnd=0x%08X)\n"), pBar->GetSafeHwnd());
	ASSERT(pBar);
...
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::SetCallbackHyperlink()\n"));
...
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::SetNotify\n"));
...
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::SetSize(nSizeIndex = %d, nValue = %d)\n"), nSizeIndex, nValue);
...
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::GetSize(nSizeIndex = %d)\n"), nSizeIndex);
	if (nSizeIndex >= PPTTSZ_MAX_SIZES)
		return 0;
...
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::SetDefaultSizes()\n"));
...
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::SetDirection(nDirection = %d)\n"), dwDirection);
...
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::GetDirection()\n"));
...
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::ShowHelpTooltip\n"));

	m_ptOriginal = CPoint(pt->x, pt->y);
	ti.nBehaviour = m_dwBehaviour | 
...
void CPPToolTip::ShowNormalTooltip(LPPOINT pt, PPTOOLTIP_INFO & ti)
{
	CPPTOOLTIP_TRACE(_T("CPPToolTip::ShowNormalTooltip\n"));

	m_ptOriginal = CPoint(pt->x, pt->y);
	ti.nBehaviour = PPTOOLTIP_DISABLE_AUTOPOP | PPTOOLTIP_TRACKING_MOUSE;
	SetNewTooltip(NULL, ti, FALSE, PPTOOLTIP_NORMAL);
} //End ShowNormalTooltip
...
//+++hd - from a post by profix898
void CPPToolTip::ShowMessageTooltip(LPCTSTR lpszText, UINT iIcon, int iCtrl)
{
	CRect rect;
	if(iCtrl == 0)
		AfxGetMainWnd()->GetWindowRect(&rect);
	else
		GetDlgItem(iCtrl)->GetWindowRect(&rect);
	...
	ShowHelpTooltip(&pt, lpszText, hIcon);
}
