...
#pragma warning(push, 3)			//+++hd
#include <vector>
#include <map>
#pragma warning(pop)

// +++hd  message sent to parent when virtual key code seen
extern UINT WM_PPTOOLTIP_VIRTUAL_KEY_CODE;
...
#define PPTOOLTIP_CLASSNAME		_T("XSuperTooltip")	// +++hd  // Window class name
...
typedef struct tagPPTOOLTIP_INFO
{
	...
	UINT		nVirtualKeyCode;	// +++hd  virtual control code (VK_F1, etc.)
	UINT		nKeyCodeId;		// +++hd  id that is sent to parent when 
								// nVirtualKeyCode is seen
	BOOL		bSuperTooltip;	// +++hd  TRUE = super tooltip
	HWND		hWnd;			// +++hd
...
public:
	...
	// Operations
protected:
	BOOL m_bIgnoreEnabledStatus;	//+++hd
	BOOL m_bMouseCaptured;			//+++hd
...
public:
	...
	void AddTool(CWnd * pWnd, DWORD dwIdString, LPCRECT lpRectBounds = NULL, 
		DWORD dwIDTool = 0);
	...
	void AddTool(CWnd * pWnd, DWORD dwIdString, HICON hIcon, LPCRECT lpRectBounds = NULL, 
		DWORD dwIDTool = 0);
	void AddTool(CWnd * pWnd, DWORD dwIdString, DWORD dwIdIcon, CSize & szIcon = CSize(0, 0), 
		LPCRECT lpRectBounds = NULL, DWORD dwIDTool = 0);
	void AddTool(CWnd * pWnd, LPCTSTR lpszString, HICON hIcon, LPCRECT lpRectBounds = NULL, 
		DWORD dwIDTool = 0);
	void AddTool(CWnd * pWnd, LPCTSTR lpszString, DWORD dwIdIcon, 
		CSize & szIcon = CSize(0, 0), LPCRECT lpRectBounds = NULL, DWORD dwIDTool = 0);
	void AddTool(CWnd * pWnd, LPCTSTR lpszString, DWORD dwIdBitmap, 
		COLORREF crMask, CSize & szBitmap = CSize(0, 0), LPCRECT lpRectBounds = NULL, 
		DWORD dwIDTool = 0);
	...
	void ShowNormalTooltip(LPPOINT pt, PPTOOLTIP_INFO & ti);
	void HideTooltip();
...
//	void EnableTextWrap(BOOL bEnable = TRUE){m_drawer.EnableTextWrap(bEnable);};
	...
	int  GetMaxTipWidth(){ return m_drawer.GetMaxWidth();};		// +++hd
	...
	// +++hd  ================================================================
	void SetUseCustomFont(BOOL bVal)    { m_drawer.SetUseCustomFont(bVal);  };
	void SetCustomFont   (LOGFONT fVal) { m_drawer.SetCustomFont(fVal);     };
	LOGFONT GetCustomFont()             { return m_drawer.GetCustomFont();  };

	void SetIgnoreEnabledStatus(BOOL bVal) { m_bIgnoreEnabledStatus = bVal; };
	BOOL GetIgnoreEnabledStatus()          { return m_bIgnoreEnabledStatus; };
	void ShowMessageTooltip(LPCTSTR lpszText, UINT iIcon, int iCtrl = 0);
	// +++hd  ================================================================
