...
//=============================================================================
{
	...
	if (pXTCD && !str.IsEmpty())
	{
		...
		pDC->DrawText(str, &rectOut, uFormat | DT_CALCRECT | DT_NOCLIP | DT_WORDBREAK);
		pDC->DrawText(str, &rectOut, uFormat | DT_NOCLIP | DT_WORDBREAK);
		//rectOut.InflateRect(m_nPadding, 0);
		nWidth = rectOut.right;

		pXTCD->ds.nRightX = rectOut.right;
		
		rect.bottom = rectOut.bottom;
...
//=============================================================================
{
	...
	//pXTCD->ds.uFormat = DT_VCENTER | /*DT_SINGLELINE |*/ DT_LEFT | DT_NOPREFIX | DT_NOCLIP | DT_WORDBREAK;
...
//=============================================================================
{
	...
	// test
	//it is updated expanded  an item 
	TVITEMEX tviEx = { 0, };
	tviEx.mask = TVIF_INTEGRAL | TVIF_TEXT;
	tviEx.hItem = hItem;

	CString strDummy = GetItemText(hItem);
	tviEx.pszText = (LPTSTR)(LPCTSTR)strDummy;

	//an amount of lines = an amount of lines 
	CRect rect(0, 0, 0, 0);
	GetDC()->DrawText(strDummy, &rect, DT_CALCRECT | DT_WORDBREAK);

	// if the text is wider than the tree item label, adjust accordingly.
	CRect rcItem;
	GetItemRect(hItem, &rcItem, TRUE);

	int integral = GetIndentLevel(hItem);
	TRACE("TEXT: %s RECT LEFT SIZE: %d\n", strDummy, rcItem.left);

	if (integral >= 1)
	{
		int size = strDummy.GetLength();
		if (size > 10)
			integral += size / 15;


		tviEx.iIntegral = integral;

		SetItem((TVITEM *)&tviEx);
	}
	// fim test
...
BOOL CXHtmlTree::OnRclick(NMHDR* pNMHDR, LRESULT* pResult)
{ 	
...
//=============================================================================
void CXHtmlTree::OnContextMenu(CWnd* pWnd, CPoint point)
//=============================================================================
{
	CPoint cp(point);
	...
	//if (m_pSelected != NULL)
	//{
	//	XHTMLTREEDATA *pXTCD = GetItemDataStruct(m_pSelected);

	//	if (pXTCD && !pXTCD->bSeparator)		//+++1.6
	//	{
	//		//SetCheck(m_pSelected, !pXTCD->bChecked);
	//	}
	//}
	...
	// Node title
	CString dummy;
	dummy.Format("%s", GetItemText(m_pSelected));
	ccmPopUp.AppendMenuItem(MF_ENABLED, 0, dummy, _T(""), pDC);
	ccmPopUp.AppendMenuItem(MF_SEPARATOR, 1, _T(""), _T(""), pDC);

	// Node related items
	/*ccmPopUp.AppendMenuItem(MF_ENABLED, APPBAR_INSERTMONTH, _T("Insert Month after"), _T(""), pDC);
	ccmPopUp.AppendMenuItem(nFlag, APPBAR_INSERTTASK, _T("Insert Task after"), _T(""), pDC);
	ccmPopUp.AppendMenuItem(nFlag, APPBAR_DELETENODE, _T("Delete Node"), _T(""), pDC);
	ccmPopUp.AppendMenuItem(nFlag, APPBAR_MODIFYNODETEXT, _T("Modify Node Text"), _T(""), pDC);

	ccmPopUp.AppendMenuItem(MF_SEPARATOR, 2, _T(""), _T(""), pDC);

	// Connecting lines related items
	ccmPopUp.AppendMenuItem(MF_ENABLED, APPBAR_SAVETASKTREE, _T("Save as a file"), _T(""), pDC);
	ccmPopUp.AppendMenuItem(MF_ENABLED, APPBAR_INSERTTASK, _T("Load from a file"), _T(""), pDC);*/
	...
	if (pBuf)
	{
		...
		if (rc)
		{
			...
			if (ret)		// useless, always returns TRUE
			{
				...
				LoadXml(xml, xml.GetRootElement(), 0/*hRoot*/, m_nXmlCount);
...
//=============================================================================
HTREEITEM CXHtmlTree::InsertXmlItem(CXmlElement *pElement, HTREEITEM hParent, bool showCheckbox)
//=============================================================================
{
	...
	if (pElement)
	{
		...
		if (/*!strName.IsEmpty() &&*/ strName.Compare(_T("...")) != 0)
		{
			...
			if (hItem)
			{
				...
				//vinicius
				if (!showCheckbox || hParent == 0/**m_phRoot*/)
				{
					SetItemState(hItem, INDEXTOSTATEIMAGEMASK(0), TVIS_STATEIMAGEMASK);
				}
			}
